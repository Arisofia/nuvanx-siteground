...
const report = { baseUrl, expectedSha: expectedSha || null, generatedAt: new Date().toISOString(), summary: { routes: config.routes.length, runs: results.length, critical: critical.length, warnings: warnings.length, result: critical.length ? 'FAIL' : warnings.length ? 'PASS_WITH_WARNINGS' : 'PASS' }, findings, results };
