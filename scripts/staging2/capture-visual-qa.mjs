...
const pages = [
  ...
  ['/papada-definicion-mandibular-madrid/', 'Papada y mandíbula: a veces es grasa, a veces es piel.'],
...
const findings = [];
const report = { base_url: baseUrl, expected_sha: expectedSha, generated_at: new Date().toISOString(), chrome: '', pages: [], navigation: {}, findings };
...
async function fetchWithRetry(url) {
  ...
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    ...
    if (response.status === 200 && !/403\s*-\s*Forbidden|Access to this page is forbidden/i.test(lastBody)) return;
...
async function loadPage(port, url, viewport) {
  ...
  await loaded;
  await session.evaluate(`new Promise((resolve) => { const finish = () => setTimeout(resolve, 600); if (document.fonts?.ready) document.fonts.ready.then(finish, finish); else finish(); })`);
  return session;
}
...
async function captureFullPage(session, destination, viewport) {
  ...
  await session.send('Emulation.setDeviceMetricsOverride', { width: viewport.width, height: fullHeight, deviceScaleFactor: 1, mobile: viewport.mobile, screenWidth: viewport.width, screenHeight: fullHeight });
  await sleep(200);
  const screenshot = await session.send('Page.captureScreenshot', { format: 'png', fromSurface: true, captureBeyondViewport: true, clip: { x: 0, y: 0, width: viewport.width, height: fullHeight, scale: 1 } }, 30000);
...
async function auditDesktopNavigation(port) {
  ...
  try {
    ...
    const initial = await session.evaluate(`(() => {
      ...
      const item = Array.from(document.querySelectorAll('.nvx-nav > .nvx-nav__list > li')).find((node) => Array.from(node.children).some((child) => child.tagName === 'A' && /protocolos signature/i.test(child.textContent)));
...
async function auditMobileNavigation(port) {
  ...
  try {
    ...
    const signatureOpened = await session.evaluate(`(() => { const nav=document.getElementById('nvx-mobile-nav'); const item=Array.from(nav?.querySelectorAll('.nvx-mobile-nav__list > li')||[]).find((node)=>Array.from(node.children).some((child)=>child.tagName==='A'&&/protocolos signature/i.test(child.textContent))); const toggle=item?Array.from(item.children).find((child)=>child.classList?.contains('nvx-mobile-nav__toggle')):null; toggle?.click(); return !!toggle; })()`);
    if (!signatureOpened) throw new Error('Protocolos Signature mobile accordion toggle was not found.');
    await sleep(300);
    const contourOpened = await session.evaluate(`(() => { const nav=document.getElementById('nvx-mobile-nav'); const signature=Array.from(nav?.querySelectorAll('.nvx-mobile-nav__list > li')||[]).find((node)=>Array.from(node.children).some((child)=>child.tagName==='A'&&/protocolos signature/i.test(child.textContent))); const submenu=signature?Array.from(signature.children).find((child)=>child.classList?.contains('sub-menu')):null; const contour=Array.from(submenu?.children||[]).find((node)=>Array.from(node.children).some((child)=>child.tagName==='A'&&/contour architecture/i.test(child.textContent))); const toggle=contour?Array.from(contour.children).find((child)=>child.classList?.contains('nvx-mobile-nav__toggle')):null; toggle?.click(); return !!toggle; })()`);
...
const chrome = spawn(chromePath, ['--headless=new', '--no-sandbox', '--disable-dev-shm-usage', '--disable-gpu', '--no-first-run', '--no-default-browser-check', '--hide-scrollbars', '--remote-allow-origins=*', `--remote-debugging-port=${port}`, `--user-data-dir=${profileDir}`, `--user-agent=${userAgent}`, 'about:blank'], { stdio: ['ignore', 'pipe', 'pipe'] });
let chromeStderr = '';
