...
define( 'NVX_THEME_VERSION', '1.8.6-prod-brand-treatment-pages-fix' );
...
function nvx_theme_scripts() {
	...
	wp_enqueue_style(
		...
		$css . 'nvx-base.css',
		array( 'nvx-tokens' ),
		$asset_version( 'nvx-base.css' )
	);

	wp_enqueue_style(
		'nvx-components',
		$css . 'nvx-components.css',
		array( 'nvx-base' ),
		$asset_version( 'nvx-components.css' )
	);
	...
	wp_enqueue_style(
		...
		$css . 'nvx-pages.css',
		array( 'nvx-components' ),
		$asset_version( 'nvx-pages.css' )
	);

	/*
	 * Gutenberg únicamente en páginas interiores genéricas.
	 */
	$layout_dependencies = array( 'nvx-pages' );

	if ( $is_generic_page ) {
		wp_enqueue_style(
			'nvx-gutenberg-pages',
			$css . 'nvx-gutenberg-pages.css',
			array( 'nvx-pages' ),
			$asset_version( 'nvx-gutenberg-pages.css' )
		);

		$layout_dependencies = array( 'nvx-gutenberg-pages' );
	}

	wp_enqueue_style(
		'nvx-site-layout',
		$css . 'nvx-site-layout.css',
		$layout_dependencies,
		$asset_version( 'nvx-site-layout.css' )
	);

	$visual_dependencies = array( 'nvx-site-layout' );

	/*
	 * Páginas interiores y shells secundarios.
	 */
	if (
		$is_p0
		|| $is_generic_page
		...
		wp_enqueue_style(
			'nvx-secondary-pages',
			$css . 'nvx-secondary-pages.css',
			array( 'nvx-site-layout' ),
			$asset_version( 'nvx-secondary-pages.css' )
		);

		$visual_dependencies[] = 'nvx-secondary-pages';
	}

	/*
	 * Formularios solo donde realmente existen formularios.
	 */
	if ( $is_form_page ) {
		wp_enqueue_style(
			'nvx-forms',
			$css . 'nvx-forms.css',
			array( 'nvx-components' ),
			$asset_version( 'nvx-forms.css' )
		);

		$visual_dependencies[] = 'nvx-forms';
	}

	/*
	 * Blog, categorías, búsquedas y entradas individuales.
	 */
	if ( $is_post_context ) {
		wp_enqueue_style(
			'nvx-posts',
			$css . 'nvx-posts.css',
			array( 'nvx-components' ),
			$asset_version( 'nvx-posts.css' )
		);

		$visual_dependencies[] = 'nvx-posts';
	}

	wp_enqueue_style(
		'nvx-visual-system',
		$css . 'nvx-visual-system.css',
		array_values( array_unique( $visual_dependencies ) ),
		$asset_version( 'nvx-visual-system.css' )
	);

	wp_enqueue_style(
		'nvx-typography-alignment',
		$css . 'nvx-typography-alignment.css',
		array( 'nvx-visual-system' ),
		$asset_version( 'nvx-typography-alignment.css' )
	);

	/*
	 * Estilos exclusivos de sedes.
	 */
	if ( $is_sede_page ) {
		wp_enqueue_style(
			'nvx-sede-page',
			$css . 'nvx-sede-page.css',
			array( 'nvx-secondary-pages' ),
			$asset_version( 'nvx-sede-page.css' )
		);
	}

	/*
	 * Portada editorial.
	 * La portada actual utiliza
	 * nvx-brand-home.css.
	 */
	if ( $is_front_page ) {
		wp_enqueue_style(
			'nvx-responsive',
			$css . 'nvx-responsive.css',
			array( 'nvx-visual-system' ),
			$asset_version( 'nvx-responsive.css' )
		);

		wp_enqueue_style(
			'nvx-brand-home',
			$css . 'nvx-brand-home.css',
			array( 'nvx-responsive' ),
			$asset_version( 'nvx-brand-home.css' )
		);
		...
		wp_enqueue_style(
			'nvx-brand-treatment-core',
			$css . 'nvx-brand-treatment-core.min.css',
			array( 'nvx-typography-alignment' ),
			$asset_version( 'nvx-brand-treatment-core.min.css' )
		);
		...
		if ( '' !== $treatment_addon ) {
			wp_enqueue_style(
				$treatment_addon,
				$css . $treatment_addon . '.min.css',
				array( 'nvx-brand-treatment-core' ),
				$asset_version( $treatment_addon . '.min.css' )
			);
		}
	} elseif ( nvx_theme_uses_brand_system() ) {
		wp_enqueue_style(
			'nvx-brand-system',
			$css . 'nvx-brand-system.min.css',
			array( 'nvx-typography-alignment' ),
			$asset_version( 'nvx-brand-system.min.css' )
		);
	...
	/*
	 * Línea visual NUVANX Fluid Organic 2026.
	 * Se carga al final para transformar la presentación sin tocar
	 * el contenido, SEO, formularios ni integraciones.
	 */
	wp_enqueue_style(
		'nvx-fluid-organic-2026',
		$css . 'nvx-fluid-organic-2026.css',
		array(),
		$asset_version( 'nvx-fluid-organic-2026.css' )
	);

	wp_enqueue_script(
		'nvx-main',
		get_template_directory_uri() . '/assets/js/nvx-main.js',
		array(),
		$ver,
		true
	);
