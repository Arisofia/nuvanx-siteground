...
function nvx_protocol_pages_catalog(): array {
	return array(
		...
		'post-maternity' => array(
			...
			'title'         => 'El posparto no es un problema único.',
			'kicker'        => 'PROTOCOLO SIGNATURE NUVANX',
			'lead'          => 'NUVANX Post-Maternity Contour™ es un abordaje médico que clasifica y trata de forma independiente la grasa localizada rebelde, la flacidez cutánea profunda, las estrías y la cicatriz de cesárea.',
			'description'   => 'No existe un "mommy makeover" láser universal. Desglosamos tu abdomen en componentes clínicos para aplicar la tecnología exacta que necesita tu tejido, siempre tras valoración médica y respetando los tiempos de recuperación biológica.',
			'review_status' => 'approved_for_publication',
		),
	);
}
...
function nvx_protocol_pages_couture_sculpt_markup( array $data ): string {
	...
	$html .= '<p>' . esc_html__( 'El abdomen no termina en el abdomen. El brazo no puede valorarse sin su relación con la axila y el torso. La espalda, la cintura y los flancos forman una misma unidad visual. Por eso, en NUVANX estudiamos cada zona valorando su integración global (espacio negativo y proporción). El objetivo no es cambiar tu cuerpo por otro, sino perfeccionar su arquitectura con la mayor naturalidad.', 'nuvanx-medical' ) . '</p>';
	
	$html .= '<div class="nvx-card-diagnostic-wrap">';
	$html .= '<h3>' . esc_html__( 'Cartografía Anatómica NUVANX', 'nuvanx-medical' ) . '</h3>';
	$html .= '<p>' . esc_html__( 'Antes de proponer tecnología, el médico analiza: Distribución de la grasa localizada, calidad y capacidad de retracción de la piel, proporción y transición armónica entre zonas, asimetrías anatómicas y límites reales de una intervención médico-estética.', 'nuvanx-medical' ) . '</p>';
	$html .= '</div>';
	$html .= '</section>';
...
function nvx_protocol_pages_post_maternity_markup( array $data ): string {
	...
	$html .= '<section class="nvx-brand-section">';
	$html .= '<h2>' . esc_html__( 'Protocolo en construcción clínica', 'nuvanx-medical' ) . '</h2>';
	$html .= '<p>' . esc_html__( 'La documentación extendida de este protocolo está en fase de despliegue web. La atención médica en consulta ya está disponible.', 'nuvanx-medical' ) . '</p>';
	$html .= '</section>';
	...
	foreach ( nvx_protocol_pages_catalog() as $key => $page ) {
		...
		wp_insert_post(
			array(
				...
				'post_name'    => $page['slug'],
				'post_content' => '<!-- NUVANX_PROTOCOL_PAGE:' . $key . ' -->',
			),
			true
		);
	}
}
add_action( 'init', 'nvx_seed_protocol_pages', 32 );
