...
function nvx_cases_page_is_current(): bool {
	if ( ! is_page() ) {
		return false;
	}
...
function nvx_cases_page_enqueue_assets(): void {
	if ( ! nvx_cases_page_is_current() ) {
		return;
	}
...
add_action( 'wp_enqueue_scripts', 'nvx_cases_page_enqueue_assets', 35 );

/** Add a stable page-state class for scoped layout ownership. */
function nvx_cases_page_body_class( array $classes ): array {
	if ( nvx_cases_page_is_current() ) {
...
add_filter( 'body_class', 'nvx_cases_page_body_class' );
...
function nvx_cases_page_replace_content( $content ) {
	if ( is_admin() || ! nvx_cases_page_is_current() || ! in_the_loop() || ! is_main_query() ) {
		return $content;
	}

	return nvx_cases_page_markup();
}
add_filter( 'the_content', 'nvx_cases_page_replace_content', 120 );
