...
async function detectInjections() {
  ...
  const injectionAnalysis = await page.evaluate(() => {
    ...
    const sgAssets = document.querySelectorAll('link[href*="siteground"], script[src*="siteground"]');
    results.siteground.optimizerAssets = sgAssets.length > 0;
    
    const sgCombined = document.querySelectorAll('link[href*="siteground-optimizer-combined"]');
    results.siteground.optimizerCombined = sgCombined.length > 0;
    
    // Check for SGO cache classes
    const sgoClasses = document.querySelectorAll('[class*="sgo-"], [id*="sgo-"]');
    results.siteground.sgoClasses = sgoClasses.length > 0;
    
    // Check for WordPress block blocks
    results.wordpress.blockBlocks = document.querySelectorAll('.wp-block').length > 0;
    
    // Check for WordPress block stylesheets
    const blockStyles = document.querySelectorAll('link[href*="wp-includes/css/dist/block-library"]');
  ...
  const sgResources = injectionAnalysis.externalResources.filter(r => 
    r.src?.includes('siteground') || r.href?.includes('siteground')
  );
  ...
  const wpBlockResources = injectionAnalysis.externalResources.filter(r => 
    r.src?.includes('wp-includes') || r.href?.includes('wp-includes')
  );
  ...
  const inlineStyleOverrides = await page.evaluate(() => {
    ...
    styleElements.forEach(style => {
      ...
      if (content.includes('color:') || content.includes('background:') || content.includes('background-color:')) {
        overrides++;
      }
    });
  ...
  console.log(`   Inline styles with color/background properties: ${inlineStyleOverrides}`);
