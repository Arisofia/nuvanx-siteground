...
function nvx_valoracion_hubspot_bootstrap_markup(): string {
	...
	return '<script id="nvx-valoracion-form-eager">(function(){"use strict";var cfg=' . $config . ';'
		. 'var formReady=false;'
		. 'function hasMarketingConsent(){var consented=false;try{if(typeof window.cmplz_has_consent==="function"){consented=!!window.cmplz_has_consent("marketing");}else if(typeof window.wp_has_consent==="function"){consented=window.wp_has_consent("marketing")===true;}}catch(e){consented=false;}return consented;}'
		. 'function isAllowedHubSpotHost(hostname){hostname=String(hostname||"").toLowerCase();return hostname==="hsforms.net"||hostname.slice(-12)===".hsforms.net"||hostname==="hsforms.com"||hostname.slice(-12)===".hsforms.com"||hostname==="hubspot.com"||hostname.slice(-12)===".hubspot.com";}'
		...
		. 'function isRenderable(root){if(!root||!hasMarketingConsent()){return false;}if(root.querySelector(".hbspt-form input,.hbspt-form textarea,.hs-form input")){return true;}if(!formReady){return false;}var iframes=root.querySelectorAll("iframe");for(var i=0;i<iframes.length;i++){if(iframeIsHubSpot(iframes[i])){return true;}}return false;}'
		...
		. 'function sync(host,frame){if(!host){return;}var live=isRenderable(frame)||isRenderable(host);var form=host.querySelector("[data-nvx-direct-form]");if(live&&!formIsDirty(form)){host.classList.add("nvx-hubspot-is-live");}else{host.classList.remove("nvx-hubspot-is-live");}}'
		. 'function hasActiveEmbed(){var scripts=document.querySelectorAll("#nvx-hubspot-forms-runtime,script[data-nvx-hubspot-canonical=\\"1\\"],script[src*=\\"/forms/embed/\\"]");for(var i=0;i<scripts.length;i++){var type=(scripts[i].getAttribute("type")||"text/javascript").toLowerCase();if(type==="text/plain"){continue;}if((scripts[i].getAttribute("data-category")||"").toLowerCase()==="marketing"&&!hasMarketingConsent()){continue;}return true;}return false;}'
		...
		. 'for(i=0;i<frames.length;i++){if(isRenderable(frames[i])){frame=frames[i];break;}}'
		. 'if(!frame){for(i=0;i<frames.length;i++){if(hasUsableHubSpotIframe(frames[i])){frame=frames[i];break;}}}'
		...
		. 'if(typeof MutationObserver==="function"&&!frame.dataset.nvxFallbackObserver){frame.dataset.nvxFallbackObserver="1";new MutationObserver(function(){sync(host,frame);}).observe(frame,{childList:true,subtree:true,attributes:true,attributeFilter:["src","data-category"]});}'
		. 'if(isRenderable(frame)||!hasMarketingConsent()){return;}'
		. 'try{host.dispatchEvent(new Event("focusin",{bubbles:true}));}catch(e){}'
		. 'window.setTimeout(function(){sync(host,frame);if(isRenderable(frame)||!hasMarketingConsent()){return;}'
		...
		. 'window.addEventListener("hs-form-event:on-ready",function(){formReady=true;boot();});'
		. 'window.addEventListener("message",function(event){var data=event&&event.data;if(typeof data==="string"){try{data=JSON.parse(data);}catch(e){data=null;}}if(!data||data.type!=="hsFormCallback"){return;}var name=String(data.eventName||"").toLowerCase();if(name==="onformready"||name==="onformsubmitted"){formReady=true;boot();}});})();</script>';
}
