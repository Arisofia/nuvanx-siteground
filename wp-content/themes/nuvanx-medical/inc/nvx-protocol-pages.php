...
function nvx_protocol_pages_catalog(): array {
	return array(
		'couture-sculpt' => array(
			...
			'description'   => 'Entendemos la medicina estética como un ejercicio de quiet luxury: una indicación individualizada y orientada a una evolución discreta, donde la coherencia de la transición importa tanto como el objetivo clínico. Couture Sculpt™ materializa esta visión dentro de los límites definidos en la valoración médica.',
...
function nvx_protocol_pages_couture_sculpt_markup( array $data ): string {
	...
	$html .= '<p>' . esc_html__( 'La indicación se define según la anatomía, el tejido predominante y los límites del procedimiento. El protocolo clasifica la necesidad clínica en tres posibles vías de actuación:', 'nuvanx-medical' ) . '</p>';
	...
	$html .= '<li><strong>RETRAER:</strong> ' . esc_html__( 'Cuando existe una indicación clara para actuar sobre la laxitud cutánea y la calidad del tejido.', 'nuvanx-medical' ) . '</li>';
	$html .= '</ul>';
	$html .= '</section>';
	...
	$html .= '<p><strong>' . esc_html__( 'Piernas y Tren Inferior', 'nuvanx-medical' ) . '</strong><br>' . esc_html__( 'Estudiamos continuidad, laxitud y proporción para definir una mejora proporcionada del contorno.', 'nuvanx-medical' ) . '</p>';
	$html .= '</section>';

	$html .= '<section class="nvx-brand-section">';
	$html .= '<h2>' . esc_html__( 'Couture Sculpt™: El protocolo y la tecnología', 'nuvanx-medical' ) . '</h2>';
	$html .= '<p>' . esc_html__( 'NUVANX Contour Architecture™ se articula a través del protocolo médico Couture Sculpt™. En lugar de depender de una sola máquina, el médico selecciona la modalidad que corresponde a la anatomía y al objetivo clínico.', 'nuvanx-medical' ) . '</p>';
	$html .= '<ul class="nvx-check-list">';
	$html .= '<li><strong>Endoláser Corporal / Endolift®:</strong> ' . esc_html__( 'Para abordar grasa localizada y laxitud mediante energía láser cuando la exploración médica lo indique.', 'nuvanx-medical' ) . '</li>';
	$html .= '<li><strong>Radiofrecuencia Fraccionada (EXION® Body):</strong> ' . esc_html__( 'Para apoyar la firmeza y la calidad tisular según la indicación y el plan médico.', 'nuvanx-medical' ) . '</li>';
	$html .= '<li><strong>' . esc_html__( 'Protocolos combinados:', 'nuvanx-medical' ) . '</strong> ' . esc_html__( 'Para integrar distintas modalidades cuando exista una justificación clínica documentada.', 'nuvanx-medical' ) . '</li>';
	$html .= '</ul>';
	$html .= '</section>';
	...
	$html .= '<p>' . esc_html__( 'El protocolo no se indica cuando la exploración muestra que otra vía ofrece una respuesta más adecuada o segura.', 'nuvanx-medical' ) . '</p>';
	...
	$html .= '<li>' . esc_html__( 'Cuando existe sospecha de diástasis o hernia abdominal que requiera valoración o derivación específica.', 'nuvanx-medical' ) . '</li>';
	$html .= '</ul>';
	$html .= '</section>';

	$html .= '<section class="nvx-brand-section">';
	$html .= '<h2>' . esc_html__( 'Tu primera valoración clínica', 'nuvanx-medical' ) . '</h2>';
	$html .= '<p>' . esc_html__( 'El objetivo es mejorar la continuidad del contorno con una intervención proporcionada y médicamente defendible. Todo comienza con una valoración exhaustiva en Chamberí o Goya.', 'nuvanx-medical' ) . '</p>';
