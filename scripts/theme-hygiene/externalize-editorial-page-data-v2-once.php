...
function nvx_aesthetic_resolve_treatment_url(string $primary, array $alts = array()): string {
    ...
    return '@nvx-aesthetic-url:' . base64_encode($payload);
}

/** @return array{start:int,end:int,expression:string} */
function nvx_editorial_v2_assignment(string $source, string $variable): array {
...
/** @return array<mixed> */
function nvx_editorial_v2_evaluate(string $expression, string $label): array {
...
function nvx_editorial_v2_write_json(string $path, array $data): void {
    $json = json_encode(
        $data,
 ...
 */
function nvx_editorial_v2_externalize(
    string $root,
    string $relativePath,
    string $jsonFilename,
    array $assignments,
    string $anchorFunction,
    string $helperCode,
    ...
    foreach ($assignments as $variable => $key) {
        $assignment = nvx_editorial_v2_assignment($source, $variable);
        $catalog[$key] = nvx_editorial_v2_evaluate($assignment['expression'], $relativePath . ':' . $variable);
        $replacements[] = array(
    ...
    $functionOffset = strpos($source, $anchorFunction);
    if (false === $functionOffset) {
        throw new RuntimeException("Anchor function not found in {$relativePath}");
    }
    $docOffset = strrpos(substr($source, 0, $functionOffset), '/**');
    $insertOffset = false === $docOffset ? $functionOffset : $docOffset;
    $source = substr($source, 0, $insertOffset)
        . $helperCode
        . "\n\n"
        . substr($source, $insertOffset);

    nvx_editorial_v2_write_json(
