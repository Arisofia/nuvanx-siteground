...
function nvx_signature_phase_markup( array $page ): string {
	...
	$html      .= '<p><a class="nvx-brand-btn nvx-btn--primary" href="' . $valoracion . '">' . esc_html__( 'Iniciar valoración médica', 'nuvanx-medical' ) . '</a> <a class="nvx-brand-inline-link" href="' . esc_url( home_url( '/protocolos-signature/' ) ) . '">' . esc_html__( 'Explorar Protocolos Signature', 'nuvanx-medical' ) . '</a></p></div></section></article>';
	return $html;
}
add_action(
	'after_setup_theme',
...
function nvx_signature_hub_shell_open( array $hub ): string {
	...
	$html      .= '<p><a class="nvx-brand-btn nvx-btn--primary" href="' . $valoracion . '">' . esc_html__( 'Solicitar valoración médica', 'nuvanx-medical' ) . '</a>';
	if ( 'index' !== ( $hub['kind'] ?? '' ) ) {
	...
	$html .= '</p>';
	$html .= '<p class="nvx-brand-microcopy">' . esc_html__( 'La indicación, la tecnología, el número de sesiones, el período de recuperación y el presupuesto se confirman después de la exploración médica.', 'nuvanx-medical' ) . '</p>';
	$html .= '</header>';
	return $html;
}
...
function nvx_signature_hub_shell_close(): string {
	...
	$html .= '<p><a class="nvx-brand-btn nvx-btn--primary" href="' . esc_url( nvx_signature_valoracion_url() ) . '">' . esc_html__( 'Iniciar valoración médica', 'nuvanx-medical' ) . '</a></p>';
