...
The obsolete `--ghost` variant is removed. Use `--light` / `--secondary-on-dark` on charcoal bands.
...
The header CTA uses the canonical primary variant and only adjusts compact navigation sizing in `nvx-header.css` via `#nvx-header-cta` (not a `.nvx-button*` selector redefinition). Icon controls such as the hamburger and close buttons, Journal pagination, topic chips, card text links and accordion summaries are separate interaction components; they must use canonical tokens but are not converted into CTA pills.

## Gate

Permanent CI: `.github/workflows/button-system-gate.yml` runs `scripts/design-system/audit-buttons.mjs`.
