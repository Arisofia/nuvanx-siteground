...
const TRANSIENT_FILESYSTEM_CODES = new Set(['EAGAIN', 'EBUSY', 'EIO', 'EMFILE', 'ENFILE', 'ENOSPC']);
...
function isAllowedSiteGroundAbort(message, documentUrl) {
  ...
  if (requestUrl.startsWith(captchaPrefix)) return true;
...
function isTransientFilesystemError(error) {
  ...
  while (current && typeof current === 'object' && !visited.has(current)) {
    visited.add(current);
    if (TRANSIENT_FILESYSTEM_CODES.has(current.code)) return true;
...
async function main() {
  ...
  await writeBlockCEvidenceBundle({
    ...
    expectedSha,
    artifactsDir,
    extraEntries: [[recoveryPath, `${JSON.stringify(recoveryEvidence, null, 2)}\n`]],
  });
...
try {
  ...
  if (isTransientFilesystemError(error)) {
    console.error(`BLOCK_C_HOME_MOBILE_RECOVERY=FAIL_TRANSIENT reason=filesystem error=${sanitizeLogValue(error instanceof Error ? error.message : error)}`);
